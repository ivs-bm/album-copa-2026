      ...
      <main className="w-full flex-1 flex flex-col px-3 py-4 space-y-4">
        ...
        {activeTab === 'jogos' && (
            <div className={`${cardBg} p-6 rounded-3xl border text-center flex-1 w-full flex flex-col items-center justify-center`}>
                ...
                {showProCode && (<div className="flex gap-2 mb-6 w-full max-w-xs"><input className="flex-1 bg-slate-700 text-white p-3 rounded-xl text-xs" onChange={(e) => setProInput(e.target.value)} /><button onClick={() => { if(proInput === 'NOSVICOPA2026') { setIsPro(true); setShowProCode(false); setToast("Modo Pro!"); } }} className="bg-emerald-500 text-white px-6 rounded-xl text-xs font-bold">OK</button></div>)}
            </div>
        )}
