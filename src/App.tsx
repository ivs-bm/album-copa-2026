...
import { 
  ...
  Globe2, Check, Cloud, LogIn, Users, LogOut, KeyRound, Lock, Copy, MapPin
} from 'lucide-react';
import { initializeApp, getApps, getApp } from 'firebase/app';
import { getAuth, signInWithPopup, GoogleAuthProvider, onAuthStateChanged, signOut } from 'firebase/auth';
import { getFirestore, doc, setDoc, updateDoc, onSnapshot } from 'firebase/firestore';

// CONFIGURAÇÃO FIREBASE (MANTIDA EQUAL)
...
// 1. ESTRUTURA DE SEÇÕES ATUALIZADA (COM GRUPOS E NOMES EM PT-BR)
const SECTIONS = [
  // SEÇÕES ESPECIAIS
  ...
  // GRUPO A (Exemplo de agrupamento realista)
...
export default function App() {
  ...
  // 2. FUNÇÃO DE SCROLL SUAVE PARA O MENU SUSPENSO
  ...
  // AGRUPAMENTO DE SEÇÕES PARA O RENDER
      ...
      <main className="max-w-5xl mx-auto px-4 lg:px-8">
            ...
            {isPro ? (
              ...
              <div className="mb-8 bg-gradient-to-r from-slate-900 to-slate-800 p-6 rounded-3xl shadow-lg border border-slate-700 text-white flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                  ...
                  <p className="text-sm text-slate-300 max-w-md">Sincronize o álbum em tempo real com sua esposa e filho. Custa menos que 2 pacotinhos (R$ 14,90) e evita comprar figurinhas repetidas à toa!</p>
                </div>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-6 mb-8">
          <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 flex flex-col items-center">
            ...
            <span className="text-[10px] md:text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Tens</span>
          </div>
          <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 flex flex-col items-center">
        ...
        {/* 2. NOVO COMPONENTE: MENU SUSPENSO DE NAVEGAÇÃO RÁPIDA */}
        <div className="mb-6 bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-3">
          <MapPin className="text-teal-600" size={24}/>
          <label htmlFor="section-select" className="text-sm font-bold text-slate-700">Ir para Seleção:</label>
          <select 
            id="section-select"
            onChange={(e) => scrollToSection(e.target.value)}
            className="flex-1 bg-slate-100 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 outline-none appearance-none"
          >
        ...
        {/* BARRA DE BUSCA E FILTROS TRADUZIDA */}
