...
// ESTRUTURA DE DADOS
// ============================================================================
// Array que constrói todas as seleções, bandeiras e figurinhas
const SECTIONS = [
...
export default function App() {
  ...
  // ============================================================================
  // ESTADO DA APLICAÇÃO (Variáveis que mudam e atualizam a tela)
  // ============================================================================
  ...
  const [stickers, setStickers] = useState({}); // Objeto com todas as figurinhas marcadas
  ...
  const [deferredPrompt, setDeferredPrompt] = useState(null); // Controle do convite de instalação PWA
  const [isStandalone, setIsStandalone] = useState(false); // Verifica se o app já está instalado
  
  const sectionsRef = useRef({}); // Referência para as âncoras (fazer o menu deslizar para o país correto)

  // ============================================================================
  // LÓGICA DE INTERAÇÃO E REGRAS DE NEGÓCIO
  // ============================================================================

  // Detecta se o aplicativo pode ser instalado (PWA)
  useEffect(() => {
  ...
  // Botão: Ação de clicar em "Instalar Aplicação"
  ...
  // Verifica o login do usuário quando o App carrega
  useEffect(() => { 
  ...
  // Busca as figurinhas em tempo real no banco de dados (Firestore)
  useEffect(() => {
  ...
  // Botão: Ação ao tocar em uma figurinha
  ...
  // Botões: Copiar Pix, Copiar Convite, Rolar Página e Cálculo de Estatísticas
  ...
  // Cálculo: Processa coladas, repetidas e faltantes
  const stats = useMemo(() => {
  ...
  // Variáveis para trocar cores baseadas no Tema
  ...
  // ============================================================================
  // RENDERIZAÇÃO PRINCIPAL (INTERFACE DO APLICATIVO LÓGADO)
  // ============================================================================
  return (
    // DIV PRINCIPAL: Envelopa todo o site
    <div className={`w-full max-w-[100vw] min-h-screen flex flex-col ${themeBg} relative overflow-x-hidden pb-20 transition-colors duration-300`}>
      ...
      {/* TOAST NOTIFICATION: Balão de aviso no topo (ex: "Pix copiado!") */}
      ...
      {/* CABEÇALHO (HEADER): Fixado no topo da tela */}
      <header className={`w-full ${isDarkMode ? 'bg-slate-950' : 'bg-gradient-to-br from-emerald-800 to-teal-700'} text-white px-4 py-3 sticky top-0 z-40 shadow-md`}>
        <div className="flex justify-between items-center mb-2">
           ...
           {/* BOTÕES DO CABEÇALHO: Informação e Modo Escuro */}
      ...
      {/* CONTEÚDO PRINCIPAL: Renderiza a tela de acordo com a aba selecionada */}
      <main className="w-full flex-1 flex flex-col px-3 py-4 space-y-4">
        
        {/* ======================================================= */}
        {/* GUIA 1: ÁLBUM DE FIGURINHAS */}
        {/* ======================================================= */}
        {activeTab === 'album' && (
            <div className="flex-1">
              
              {/* MENU HORIZONTAL DE BANDEIRAS (Scroll Lateral) */}
        ...
        {/* ======================================================= */}
        {/* GUIA 2: ESTATÍSTICAS / RESUMO (GRÁFICO DE PIZZA) */}
        {/* ======================================================= */}
        {activeTab === 'stats' && (
            <div className={`${cardBg} p-5 rounded-2xl shadow-sm border text-center flex flex-1 flex-col justify-center`}>
                ...
                {/* GRÁFICO DE PIZZA (Feito com CSS Gradient) */}
                ...
                {/* BLOCOS DE INFORMAÇÃO NUMÉRICA (Coladas, Repetidas, Faltantes) */}
      ...
      {/* ============================================================================ */}
      {/* MENU DE NAVEGAÇÃO INFERIOR (BOTTOM NAV) */}
      {/* ============================================================================ */}
